/*! @Copyright MeanThemes 2014 - JRNY for Ghost 1.0.1 */
					...
					if ( $('body').is('.post-template') ) {
						...
						var expand = function() {
										...
										$('.f-expander').on( 'click' , function(e) {
											...
											if ( galleryPresent.length > 0 ) {

												postTopImageHeight = $('.part-gallery').outerHeight();

											}

											var imageHeight = postTopImageHeight;
											...
											if ( clicked === true ) {
												...
												if ( galleryPresent.length ) {

													$('.owl-prev, .owl-next').animate( { 'top': expandHeight }, 0 );
						...
						$(window).resize(function () {
							...
							$('.with-image .post-top').css('height' , expandHeight).removeClass('expanded');
							expand();
